    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>{{ q.total_orders.toLocaleString(localeString) }}</td>
                <td>{{ currencySymbol }}{{ formatCurrency(q.total_revenue) }}</td>
                <td>{{ currencySymbol }}{{ formatCurrency(q.avg_order_value) }}</td>
                <td>
          ...
          <div class="bar-chart">
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  class="bar"
                  :style="{ height: (maxRevenue > 0 ? (month.revenue / maxRevenue) * 200 : 0) + 'px' }"
                  :title="currencySymbol + formatCurrency(month.revenue)"
                ></div>
              </div>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ month.order_count.toLocaleString(localeString) }}</td>
                <td>{{ currencySymbol }}{{ formatCurrency(month.revenue) }}</td>
                <td>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-value">{{ currencySymbol }}{{ formatCurrency(totalRevenue) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.summary.avgMonthlyRevenue') }}</div>
          <div class="stat-value">{{ currencySymbol }}{{ formatCurrency(avgMonthlyRevenue) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.summary.totalOrders') }}</div>
          <div class="stat-value">{{ totalOrders.toLocaleString(localeString) }}</div>
        </div>
        <div class="stat-card">
...
// Maps zero-based month index to i18n key
...
export default {
  ...
  setup() {
    ...
    // Derive the BCP-47 locale string for toLocaleString calls
    const localeString = computed(() => currentLocale.value === 'ja' ? 'ja-JP' : 'en-US')
    ...
    // --- Computed summary stats (derived from loaded data, not stored separately) ---
    ...
    // Precomputed max revenue used by the bar chart height calculation
    ...
    // Re-load whenever any of the 4 global filters change
    ...
    const formatCurrency = (num) => {
      return num.toLocaleString(localeString.value, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      })
    }

    // Converts "YYYY-MM" to a translated month abbreviation + year, e.g. "Jan 2025"
    ...
    const getChangeValue = (current, previous) => {
      ...
      if (change > 0) return '+' + currencySymbol.value + formatCurrency(change)
      if (change < 0) return '-' + currencySymbol.value + formatCurrency(Math.abs(change))
